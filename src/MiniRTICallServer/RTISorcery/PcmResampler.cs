...
internal static class PcmResampler
{
    public static short[] Resample(short[] pcm, int inRate, int outRate)
    {
        ...
        else if (inRate == 8000 && outRate == 24000)
        {
            // Crude up-sample to 24Khz by 3x each sample. This sounds bad, use for testing only.
        ...
        else if (inRate == 24000 && outRate == 8000)
        {
            // Crude down-sample to 8Khz by skipping every first and second sample.
            return pcm.Where((x, i) => i % 3 == 2).ToArray();
        }
