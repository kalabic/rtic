...
internal sealed class RTICCallAgent : ICallAgent, IAsyncDisposable
{
        ...
        RTICCallAgentOptions? options = null)
    {
        ...
        try
        {
            ...
            return new RTICCallAgent(
                configuration ?? RTICConfig.FromEnvironment(),
    ...
    {
        if (_configuration._client is null ||
            _configuration._client.Type == EndpointType.IncompleteOptions)
        {
            throw new AgentPreparationException(
                AgentPreparationFailureKind.Configuration,
                _configuration._errorMessage ?? "Realtime configuration is incomplete.");
        }
